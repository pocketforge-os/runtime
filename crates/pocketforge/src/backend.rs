...
pub trait Backend: Send + Sync {
    ...
    /// Control-plane-only preference write. Game/broker backends intentionally expose no write op.
    ...
    /// backend has no store-backed value. The default keeps non-preference backends honest; the
    /// in-process and broker backends override it, and `EnforcingBackend` delegates to its inner.
    ...
    /// one that cannot (the broker client — PFW1 v1 has reads but no subscribe op; callers should
    /// poll on resume rather than receive a fake observer). The receiver yields the new
