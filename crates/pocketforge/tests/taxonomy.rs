...
fn a133_vibration_is_cosmetic_noop_absent() {
    ...
    let vib = pf
        .acquire::<Vibration>()
        .expect("vibration acquire is infallible (cosmetic)");
...
fn a523_imu_is_hardware_absent_qmi8658_dt_present_but_unbound() {
    ...
    assert!(
        !has.hardware,
        "a523 advertises no BOUND IMU (qmi8658 driver unbound)"
    );
    assert_eq!(pf.acquire::<Imu>().err(), Some(CapError::HardwareAbsent));
    assert_eq!(pf.query::<Imu>(), PermissionState::Denied);
}
...
fn imu_present_and_readable_on_a_bound_imu() {
    ...
    assert!(
        pf.has_capability::<Imu>().present(),
        "the rig has a bound qmi8658"
    );
...
fn a523_location_absent_gnss_unbound() {
    ...
    let pf = Pf::in_process(common::descriptor("a523"));
    assert!(
        !pf.has_capability::<Location>().present(),
        "a523 omits GNSS until proven bound"
    );
    assert_eq!(
        pf.acquire::<Location>().err(),
        Some(CapError::HardwareAbsent)
...
fn synthetic_gnss_is_default_deny_consent() {
    ...
    assert!(
        pf.has_capability::<Location>().present(),
        "synthetic descriptor advertises GNSS"
    );
    ...
    assert!(
        !pf.is_granted::<Location>(),
        "default-deny ⇒ not granted (assert_capability_denied)"
    );
}
...
fn input_action_map_is_pure_descriptor_data() {
    ...
    assert!(
        m133.map().by_id("home").is_none(),
        "base Pro has no home button"
    );
    assert!(
        m523.map().by_id("home").is_some(),
        "Pro S adds a home button (descriptor row)"
    );
    assert!(
        m523.map().by_id("l3").is_some(),
...
fn leds_present_on_both_with_descriptor_count() {
    ...
    let a523 = Pf::in_process(common::descriptor("a523"));
    assert_eq!(
        a133.acquire::<Leds>().unwrap().count,
        23,
        "a133 = 23 sunxi_led"
    );
    assert!(
        a523.acquire::<Leds>().unwrap().count > 0,
        "a523 has an led array"
    );
}
...
fn descriptors_come_from_the_platform_checkout() {
    let root = pocketforge::test_support::platform_root().expect(
        "no platform checkout — see crates/pocketforge/tests/README.md (this must not skip)",
    );
    for id in ["a133", "a523"] {
        let path = pocketforge::test_support::try_descriptor_path(id).expect("descriptor path");
        assert!(
            path.starts_with(&root),
            "{id} descriptor resolved outside the platform checkout"
        );
