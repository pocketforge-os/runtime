...
fn main() {
    let dir = std::env::var_os("PF_APPOPS_DIR")
        .unwrap_or_else(|| std::env::temp_dir().join("step2-appops").into_os_string());
    ...
    println!(
        "     observed: {q:?}   {}",
        pf_check(q == PermissionState::Prompt)
    );
    ...
    println!(
        "     supervisor asks_seen.len() = {}   {}",
        sup.asks_seen().len(),
        pf_check(sup.asks_seen().len() == 1)
    );
    let ask = &sup.asks_seen()[0];
    println!(
        "     ask: app={} cap={} arg={:?} ctx={:?} default_focus={:?}",
    ...
    println!(
        "     event: {evt:?}   {}",
        pf_check(evt == PermissionState::Granted)
    );
    ...
    println!(
        "     ledger.check(location) = {c:?}   {}",
        pf_check(c == GrantCheck::OnceUsed)
    );
    ...
    println!(
        ...
        match &r2 {
            Ok(_) => "Ok".to_string(),
            Err(e) => format!("Err({})", cap_name(*e)),
        },
        pf_check(matches!(r2, Err(CapError::ConsentDenied)))
    );
    let evt = rx
        .recv_timeout(Duration::from_secs(1))
        .expect("standing-deny event");
    println!(
        "     event: {evt:?}   {}",
        pf_check(evt == PermissionState::Denied)
    );
    println!(
        "     supervisor asks_seen.len() = {} (no re-prompt)   {}",
        sup.asks_seen().len(),
        pf_check(sup.asks_seen().len() == 1)
    );
    ...
    let evt = rx
        ...
        .expect("revoke event");
    println!(
        "     inner set_consent(Denied)  → event: {evt:?}   {}",
        pf_check(evt == PermissionState::Denied)
    );
    println!(
        "     acquire after revoke: {}   {}",
        match pf.acquire::<Location>() {
            Ok(_) => "Ok (BAD)".to_string(),
            Err(e) => format!("Err({})", cap_name(e)),
        },
        pf_check(matches!(
            pf.acquire::<Location>(),
    ...
    println!(
        "     supervisor asks_seen.len() = {} (unchanged)   {}",
        sup.asks_seen().len(),
        pf_check(sup.asks_seen().len() == seen_before)
    );
    println!(
        "     query::<Entropy>() = {:?}   {}",
        pf.query::<Entropy>(),
    ...
    .unwrap()
    .validate(inner2.descriptor())
    .unwrap();
    let sup2 = Arc::new(SimulatedSupervisor::new());
    sup2.prepare(PreparedAnswer::allow_always(
        "com.step2.restart",
        "location",
        None,
    ));
    ...
    let eb2 = Arc::new(EnforcingBackend::with_consent_portal(
        ...
        Arc::new(QuotaLedger::new()),
        l2.clone(),
        sup2.clone() as Arc<dyn SupervisorAsk>,
    ...
    let eb3 = Arc::new(EnforcingBackend::with_consent_portal(
        ...
        Arc::new(QuotaLedger::new()),
        l3.clone(),
        sup3.clone() as Arc<dyn SupervisorAsk>,
        "Restart Test 2".to_string(),
    ));
    let key = GrantKey::new("com.step2.restart", "location", None);
    println!(
        "     ledger.check(location) in new session = {:?}   {}",
        l3.check(&key),
        pf_check(l3.check(&key) == GrantCheck::Always)
    );
    eb3.acquire("location").unwrap();
    println!(
        "     new-session acquire: Ok (Always grant carried across restart)   {}",
    ...
    let m4 = AppManifest::from_toml("[app]\nid = \"com.step2.noself\"\nuse = [\"input\"]\n")
        .unwrap()
        .validate(inner4.descriptor())
        .unwrap();
    ...
    let eb4 = Arc::new(EnforcingBackend::with_consent_portal(
        ...
        Arc::new(QuotaLedger::new()),
        l4.clone(),
        Arc::new(SimulatedSupervisor::new()) as Arc<dyn SupervisorAsk>,
        "No-Self".to_string(),
    ));
    inner4.set_consent("location", PermissionState::Granted);
    let r = eb4.acquire("location");
    println!(
        "     app-side set_consent(location=Granted); eb.acquire: {}   {}",
        match &r {
            Ok(_) => "Ok (BAD)".to_string(),
            Err(e) => format!("Err({})", cap_name(*e)),
        },
    ...
    println!(
        "     ledger.record_grant(vibration outside ceiling): {}",
        match &r {
            Ok(_) => "Ok (BAD)".to_string(),
            Err(e) => format!("Err({e})"),
        }
