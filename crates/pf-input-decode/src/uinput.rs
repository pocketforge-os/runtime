//! The **uinput sink** — the single VIRTUAL evdev gamepad this daemon presents. Built from a
//! [`UinputSpec`] via the legacy `write(uinput_user_dev)` + `UI_DEV_CREATE` path (the exact shape
//! `pf-input-broker`, the E5 sim's `uinput_synth.py`, and `qemu-tsp`'s passthrough all validate).
//! `Drop` destroys the device; on process exit the kernel closes the fd and destroys it anyway,
//! so a daemon killed by `SIGKILL` still leaves no orphan node.
...
/// Absolute-axis calibration (mirrors `struct input_absinfo`).
...
/// A complete virtual-device description: identity + the codes/axes it advertises.
#[derive(Debug, Clone)]
...
impl Uinput {
    ...
    fn set_bit(&self, req: libc::Ioctl, bit: libc::c_int) -> io::Result<()> {
    ...
    /// The `/dev/input/eventN` node of the device (if it could be resolved).
    ...
    pub fn syn(&self) -> io::Result<()> {
        self.emit(codes::EV_SYN, codes::SYN_REPORT, 0)
    }
}
